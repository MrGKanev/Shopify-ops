...
@section('content')
    <div class="flex flex-col gap-6">
        <x-page-header title="Push Log" subtitle="History of orders pushed to ShipStation for this store." />

        <x-card>
            <form class="flex flex-col gap-3 sm:flex-row sm:items-end" method="GET">
                <div class="min-w-0 flex-1">
                    <label class="text-sm font-medium" for="push-search">Order or Shopify ID</label>
        ...
        <x-data-table :headers="['Order', 'Shopify ID', 'ShipStation ID', 'Pushed at', '']">
            @forelse ($pushes as $push)
                <tr>
                    ...
                    <td class="px-4 py-3">
                        <a class="font-medium text-indigo-600 hover:underline dark:text-indigo-400" href="https://app.shipstation.com/#!/orders/all-orders-search-result?quickSearch={{ urlencode(ltrim($push->order_number, '#')) }}" target="_blank" rel="noopener noreferrer">View in SS</a>
                    </td>
                </tr>
            @empty
                <tr><td class="px-4 py-8 text-center text-slate-500 dark:text-slate-400" colspan="5">No pushes yet.</td></tr>
            @endforelse
        </x-data-table>
