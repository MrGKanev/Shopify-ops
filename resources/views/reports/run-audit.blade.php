...
@section('content')
    <div class="flex flex-col gap-6">
        ...
        @endif
        @if ($reportFailed)
            <x-alert tone="error">{{ __('The audit could not be completed. Check both integrations and try again.') }}</x-alert>
        @endif
        ...
        @if ($result)
            <section class="flex flex-col gap-4">
                ...
                @if ($result->shopifyTruncated)
                    <x-alert tone="warn">{{ __('Results are incomplete because Shopify data was truncated.') }}</x-alert>
                @endif

                @if ($result->duplicates !== [])
                    <x-alert tone="warn">
                        <strong>{{ count($result->duplicates) }} potential duplicate{{ count($result->duplicates) === 1 ? '' : 's' }} detected</strong> {{ __('— same email and amount within 24 hours.') }}
                ...
                <x-data-table :headers="['', 'Order', 'Date', 'Email', 'Total']">
                    ...
                    @empty
                        <tr>
                            <td class="px-4 py-8 text-center text-slate-500" colspan="5">{{ __('Every eligible Shopify order was found in ShipStation.') }}</td>
                        </tr>
                    @endforelse
                </x-data-table>
            </section>
