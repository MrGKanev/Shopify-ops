    ...
    <div class="flex flex-col gap-6">
        ...
        @if ($result)
            <section class="flex flex-col gap-4">
                ...
                @if ($result->truncated)
                    <x-alert tone="warn">Results are incomplete: events were truncated after {{ $result->pages }} pages.</x-alert>
                @endif
                ...
                <x-data-table :headers="['Order', 'Placed', 'First fulfillment', 'Changed', 'After shipment', 'Email', 'Current address', 'Total', 'Status']">
                    ...
                    @empty
                        <tr>
                            <td class="px-4 py-8 text-center text-slate-500" colspan="9">No post-ship address changes found.</td>
                        </tr>
                    @endforelse
                </x-data-table>
            </section>
