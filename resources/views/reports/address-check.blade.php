    ...
    <div class="flex flex-col gap-6">
        ...
        <form class="grid gap-4 rounded-xl border border-slate-200 bg-white p-5 sm:grid-cols-3 dark:border-slate-800 dark:bg-slate-900" method="POST" action="{{ route('reports.address-check.store') }}">
            ...
            <div class="flex flex-col justify-end gap-2">
                <label><input name="unfulfilled_only" type="checkbox" value="1" @checked(old('unfulfilled_only', $unfulfilledOnly))> Unfulfilled only</label>
                <label><input name="po_box_only" type="checkbox" value="1" @checked(old('po_box_only', $poBoxOnly))> PO Box issues only</label>
