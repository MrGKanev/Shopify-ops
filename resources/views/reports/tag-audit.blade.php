    ...
    <div class="flex flex-col gap-6">
        ...
        @if ($result)
            <section class="flex flex-col gap-4">
                ...
                @if ($result->truncated)
                    <x-alert tone="warn">Results were truncated after {{ $result->pages }} pages. Narrow the date range for a complete inventory.</x-alert>
                @endif

                <x-data-table :headers="['Tag', 'Orders', 'Last seen', 'Last order', 'Search']">
                    @forelse ($result->tags as $row)
                        <tr class="{{ $row['orphan'] ? 'text-slate-400' : '' }}">
                            <td class="px-4 py-3"><code>{{ $row['tag'] }}</code> @if ($row['orphan'])<x-badge tone="warn" class="ml-2">orphan</x-badge>@endif</td>
                    ...
                    @empty
                        <tr>
                            <td class="px-4 py-8 text-center text-slate-500" colspan="5">No orders in this date range have tags.</td>
                        </tr>
                    @endforelse
                </x-data-table>
            </section>
