...
@section('content')
    <div class="flex flex-col gap-6">
        ...
        @endif
        @if ($reportFailed)
            <x-alert tone="error">The report could not be completed. Check Shopify access scopes and try again.</x-alert>
        @endif
        ...
        @if ($result)
            ...
            @if ($result->truncated)
                <x-alert tone="warn">Results are incomplete: Shopify fulfillment orders were truncated after {{ $result->pages }} pages.</x-alert>
            @endif
            ...
            <x-data-table :headers="['Order', 'Placed', 'Waiting', 'Hold reason', 'Notes', 'Email', 'Total', 'Financial']">
                ...
                @empty
                    <tr>
                        <td class="px-4 py-8 text-center text-slate-500" colspan="8">No on-hold fulfillment orders found.</td>
                    </tr>
                @endforelse
            </x-data-table>
        @endif
