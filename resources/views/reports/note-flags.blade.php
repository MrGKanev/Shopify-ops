...
@section('content')
    <div class="flex flex-col gap-6">
        ...
        <form class="grid gap-4 rounded-xl border border-slate-200 bg-white p-5 sm:grid-cols-4 dark:border-slate-800 dark:bg-slate-900" method="POST" action="{{ route('reports.note-flags.store') }}">
            ...
            @endforeach
            <div>
                <label class="text-sm font-medium" for="keywords">{{ __('Keywords') }}</label>
        ...
        @if ($result)
            ...
            <x-data-table :headers="['Order', 'Placed', 'Matched', 'Note', 'Email']">
                ...
                @empty
                    <tr>
                        <td class="px-4 py-8 text-center text-slate-500" colspan="5">{{ __('No order notes matched the keywords.') }}</td>
                    </tr>
                @endforelse
            </x-data-table>
        @endif
