...
@section('content')
    <div class="flex flex-col gap-6">
        ...
        <form class="grid gap-4 rounded-xl border border-slate-200 bg-white p-5 sm:grid-cols-5 dark:border-slate-800 dark:bg-slate-900" method="POST" action="{{ route('reports.high-value-no-phone.store') }}">
            ...
            @endforeach
            <div>
                <label class="text-sm font-medium" for="minimum">Minimum value</label>
            ...
            <div>
                ...
                <datalist id="currency-options"><option value="ALL">All currencies</option></datalist>
                @error('currency')
                    <p class="text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
        ...
        @if ($reportFailed)
            <x-alert tone="error">The report could not be completed. Check the Shopify integration and try again.</x-alert>
        @endif
