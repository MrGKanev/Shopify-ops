...
@section('content')
    <div class="flex flex-col gap-6">
        ...
        @endif
        @if ($loadFailed)
            <x-alert tone="error">{{ __('Metafield definitions could not be loaded.') }}</x-alert>
        @endif
        @if ($operationFailed)
            <x-alert tone="error">{{ __('The metafield operation could not be completed.') }}</x-alert>
        @endif

        <section class="flex flex-col gap-3" aria-labelledby="definitions-heading">
            <h2 class="text-xl font-bold" id="definitions-heading">{{ __('Order definitions') }}</h2>
            <x-data-table :headers="['Namespace.key', 'Name', 'Type', 'Description']">
                ...
                @empty
                    <tr><td class="px-4 py-8 text-center text-slate-500 dark:text-slate-400" colspan="4">{{ __('No definitions found.') }}</td></tr>
                @endforelse
            </x-data-table>
        </section>
        ...
        <x-card>
            <form class="grid gap-4 sm:grid-cols-3" method="POST" action="{{ route('metafields.search') }}">
                ...
                @endforeach
                <div class="flex items-end"><x-button type="submit">{{ __('Search') }}</x-button></div>
            </form>
        </x-card>
        ...
        @if ($search)
            <section class="flex flex-col gap-3" aria-labelledby="metafield-results-heading">
                ...
                @endif
                @if ($search['orders'] === [])
                    <x-empty-state>{{ __('No matching orders found.') }}</x-empty-state>
                @else
        ...
        <x-card>
            <form class="grid gap-4" method="POST" action="{{ route('metafields.lookup') }}">
                @csrf
                <h2 class="text-xl font-bold">{{ __('Inspect orders') }}</h2>
                <div>
                    <label class="text-sm font-medium" for="lookup-orders">{{ __('Order numbers') }}</label>
                    ...
                    @error('orders')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400" id="lookup-orders-error">{{ __($message) }}</p>
                    @enderror
                </div>
                <div>
                    <label class="text-sm font-medium" for="lookup-filter">{{ __('Optional namespace, key, or value filter') }}</label>
        ...
        @if ($lookup)
            <section class="flex flex-col gap-3" aria-label="{{ __('Inspected orders') }}">
