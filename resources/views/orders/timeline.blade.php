...
@section('content')
    <div class="flex flex-col gap-6">
        <section class="flex flex-col gap-2">
            ...
            <h1 class="text-3xl font-bold">{{ __('Order timeline') }}</h1>
            <p class="text-slate-500 dark:text-slate-400">{{ __('Review Shopify and ShipStation activity in one chronological view, with operational and order-risk signals.') }}</p>
        </section>
        ...
        <form class="flex flex-col gap-4 rounded-xl border border-slate-200 bg-white p-5 sm:flex-row sm:items-end dark:border-slate-800 dark:bg-slate-900" method="GET" action="{{ route('orders.timeline') }}">
            ...
            <button class="rounded-lg bg-indigo-600 px-5 py-2.5 font-semibold text-white hover:bg-indigo-500" type="submit">{{ __('Load timeline') }}</button>
        </form>

        @if ($timelineFailed)
            <x-alert tone="error">
                {{ __('The order timeline could not be completed. Check the store integrations and try again.') }}
            </x-alert>
        @endif
            ...
            @if ($result->riskScore['signals'] !== [] || $result->operationalRisks !== [])
                <section class="grid gap-4 lg:grid-cols-2">
                    <div class="rounded-xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900">
                        <h2 class="text-lg font-semibold">{{ __('Order-risk signals') }}</h2>
                        @if ($result->riskScore['signals'] === [])
                            <p class="mt-3 text-sm text-slate-500 dark:text-slate-400">{{ __('No order-risk signals detected.') }}</p>
                        @else
                    ...
                    <div class="rounded-xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900">
                        <h2 class="text-lg font-semibold">{{ __('Operational risks') }}</h2>
                        @if ($result->operationalRisks === [])
                            <p class="mt-3 text-sm text-slate-500 dark:text-slate-400">{{ __('No operational risks detected.') }}</p>
                        @else
            ...
            <section class="flex flex-col gap-4">
                ...
                @forelse ($result->timeline as $item)
                    <article class="rounded-xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900">
                        ...
                        @if ($item['url'] !== '')
                            <a class="mt-3 inline-flex text-sm font-medium text-indigo-600 hover:text-indigo-500 dark:text-indigo-400" href="{{ $item['url'] }}" target="_blank" rel="noopener noreferrer">{{ __('Open details') }}</a>
                        @endif
                    </article>
                @empty
                    <div class="rounded-xl border border-slate-200 bg-white p-5 text-slate-500 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-400">
                        {{ __('No timestamped activity is available for this order.') }}
                    </div>
                @endforelse
            </section>
        @endif
