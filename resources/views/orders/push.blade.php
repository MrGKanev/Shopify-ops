...
@section('content')
    <div class="flex flex-col gap-6">
        <section class="flex flex-col gap-2">
            <p class="text-sm font-medium text-amber-600 dark:text-amber-400">{{ __('Write actions — changes ShipStation and Shopify') }}</p>
            <h1 class="text-3xl font-bold">{{ __('Push order / fix note') }}</h1>
            <p class="text-slate-500 dark:text-slate-400">{{ __('Manually create a Shopify order in ShipStation when the automatic sync missed it, or correct an order\'s Shopify note.') }}</p>
        </section>
        ...
        <section class="flex flex-col gap-3 rounded-xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900">
            <h2 class="text-xl font-bold">{{ __('Save order note') }}</h2>
            <p class="text-sm text-slate-500 dark:text-slate-400">{{ __('Replaces the Shopify order\'s note. Look up the order first to find its numeric order ID.') }}</p>

            <form class="grid gap-3" method="POST" action="{{ route('orders.note.update') }}">
                @csrf
                <div class="grid gap-3 sm:grid-cols-2">
                    <div class="flex flex-col gap-2">
                        <label class="text-sm font-medium" for="note_order_number">{{ __('Order number') }}</label>
                        ...
                        @error('order_number')
                            <p class="text-sm text-red-600 dark:text-red-400" id="note-order-number-error" role="alert">{{ __($message) }}</p>
                        @enderror
                    </div>
                    <div class="flex flex-col gap-2">
                        <label class="text-sm font-medium" for="order_id">{{ __('Shopify order ID') }}</label>
                        ...
                        @error('order_id')
                            <p class="text-sm text-red-600 dark:text-red-400" id="order-id-error" role="alert">{{ __($message) }}</p>
                        @enderror
                    </div>
                </div>
                <div class="flex flex-col gap-2">
                ...
                <div>
                    <button class="rounded-lg bg-amber-600 px-5 py-2.5 font-semibold text-white hover:bg-amber-500" type="submit">{{ __('Save note') }}</button>
                </div>
            </form>
        </section>
    </div>
