...
@section('content')
    <div class="flex flex-col gap-6">
        <section class="flex flex-col gap-2">
            ...
            <h1 class="text-3xl font-bold">{{ __('Order compare') }}</h1>
            <p class="text-slate-500 dark:text-slate-400">{{ __('Place two Shopify orders side by side. Different values are highlighted, with ShipStation status included when configured.') }}</p>
        </section>
        ...
        <form class="grid gap-4 rounded-xl border border-slate-200 bg-white p-5 sm:grid-cols-[1fr_1fr_auto] sm:items-end dark:border-slate-800 dark:bg-slate-900" method="GET" action="{{ route('orders.compare') }}">
            ...
            <button class="rounded-lg bg-indigo-600 px-5 py-2.5 font-semibold text-white hover:bg-indigo-500" type="submit">{{ __('Compare') }}</button>
        </form>

        @if ($comparisonFailed)
            <x-alert tone="error">
                {{ __('The order comparison could not be completed. Check the store integrations and try again.') }}
            </x-alert>
        @endif
        ...
        @if ($result !== null)
            <section class="flex flex-col gap-4">
                ...
                @if ($result->shopifyMatchCountA > 1 || $result->shopifyMatchCountB > 1)
                    <x-alert tone="warn">
                        {{ __('Multiple Shopify matches were found for at least one order number. No ambiguous record was selected automatically.') }}
                    </x-alert>
                @endif
