...
@section('content')
    <div class="flex flex-col gap-6">
        <section class="flex flex-col gap-2">
            ...
            <h1 class="text-3xl font-bold">Tracking feed</h1>
            <p class="text-slate-500 dark:text-slate-400">Look up 1–30 orders and their actual ShipStation shipments.</p>
        </section>
        ...
        <form class="rounded-xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900" method="POST" action="{{ route('orders.tracking.store') }}">
            ...
            @error('orders')
                <p class="mt-2 text-sm text-red-600 dark:text-red-400" id="orders-error" role="alert">{{ $message }}</p>
            @enderror
        ...
        @if ($configurationError)
            <x-alert tone="warn">ShipStation is not configured completely for this store.</x-alert>
        @endif
        @if ($lookupFailed)
            <x-alert tone="error">Tracking could not be loaded. Try again.</x-alert>
        @endif
            ...
            <section class="flex flex-col gap-4">
                ...
                @foreach ($results as $result)
                    <article class="rounded-xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900">
                        ...
                        @if (! $result['found'])
                            <p class="mt-3 text-sm text-slate-500 dark:text-slate-400">Not found in ShipStation.</p>
                        @else
                                    ...
                                    <div class="rounded-lg bg-slate-50 p-4 text-sm dark:bg-slate-950">
                                        ...
                                        <div class="mt-2">
                                            @if ($shipment['trackingNumber'])
                                                ...
                                                @endif
                                            @else
                                                <span class="text-slate-500 dark:text-slate-400">No tracking number yet</span>
                                            @endif
                                        </div>
                                        @if ($shipment['ssUrl'])
                                            <a class="mt-2 inline-flex text-indigo-600 hover:text-indigo-500 dark:text-indigo-400" href="{{ $shipment['ssUrl'] }}" target="_blank" rel="noopener noreferrer">Open in ShipStation</a>
                                        @endif
                                    </div>
                                @endforeach
                            </div>
