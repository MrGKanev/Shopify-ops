...
<section class="w-full max-w-xl rounded-2xl border border-slate-200 bg-white p-8 shadow-sm dark:border-slate-800 dark:bg-slate-900">
    <p class="text-sm font-semibold text-indigo-600 dark:text-indigo-400">{{ __('Shopify Ops') }}</p>
    <div class="mt-3 flex items-center justify-between gap-4"><h1 class="text-3xl font-bold">{{ __('System status') }}</h1><span class="rounded-full px-3 py-1 text-sm font-semibold {{ $ready ? 'bg-emerald-100 text-emerald-800' : 'bg-red-100 text-red-800' }}">{{ $ready ? 'Operational' : 'Needs attention' }}</span></div>
