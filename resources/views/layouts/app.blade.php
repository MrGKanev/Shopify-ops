...
@endphp
<header class="mobile-header"><div class="mobile-brand"><a class="brand" href="{{ route('dashboard') }}">@if($appSettings->logoUrl())<img class="mobile-logo" src="{{ $appSettings->logoUrl() }}" alt="{{ $appSettings->displayName() }}">@else{{ $appSettings->displayName() }}@endif</a><span class="header-store">{{ $activeStore->label }}</span></div><button class="hamburger" id="js-hamburger" type="button" aria-label="{{ __('Menu') }}"><span></span><span></span><span></span></button></header>
<div class="sidebar-overlay" id="js-overlay"></div>
<div class="layout">
    <aside class="sidebar" id="js-sidebar">
        <div class="sidebar-header">
            <div class="sidebar-header-top"><a class="brand" href="{{ route('dashboard') }}">@if($appSettings->logoUrl())<img class="sidebar-logo" src="{{ $appSettings->logoUrl() }}" alt="{{ $appSettings->displayName() }}">@else{{ $appSettings->displayName() }}@endif</a><button class="theme-icon-btn" id="js-theme-toggle" type="button" title="{{ __('Toggle theme') }}"><span id="js-theme-icon">🌙</span></button></div>
            <div class="store"><span class="store-label">{{ __('Store') }}</span> {{ $activeStore->shopify_store }}</div>
            @if($availableStores->count() > 1)<form class="store-switcher" method="POST" action="{{ route('stores.active', $activeStore) }}" data-store-switcher>@csrf<select class="store-select" title="{{ __('Switch store') }}">@foreach($availableStores as $store)<option value="{{ route('stores.active', $store) }}" @selected($store->is($activeStore))>{{ $store->label }}</option>@endforeach</select></form>@endif
        </div>
        <nav class="flat-nav" aria-label="{{ __('Primary') }}">
            <a class="flat-nav-link {{ $group === 'dashboard' ? 'active' : '' }}" href="{{ route('dashboard') }}"><span class="flat-nav-icon">🏠</span><span class="nav-label">{{ __('Dashboard') }}</span></a>
            @can('run-audits')<a class="flat-nav-link {{ $group === 'audit' ? 'active' : '' }}" href="{{ route('audits.index') }}"><span class="flat-nav-icon">📋</span><span class="nav-label">{{ __('Audit') }}</span></a>@endcan
            <a class="flat-nav-link {{ $group === 'search' ? 'active' : '' }}" href="{{ route('orders.lookup') }}"><span class="flat-nav-icon">🔎</span><span class="nav-label">{{ __('Search & Lookup') }}</span></a>
            <a class="flat-nav-link {{ $group === 'manage' ? 'active' : '' }}" href="{{ route('push-logs.index') }}"><span class="flat-nav-icon">📂</span><span class="nav-label">{{ __('Manage') }}</span></a>
            @can('manage-administration')<a class="flat-nav-link {{ $group === 'settings' ? 'active' : '' }}" href="{{ route('admin.settings') }}"><span class="flat-nav-icon">⚙</span><span class="nav-label">{{ __('Settings') }}</span></a>@endcan
        </nav>
        @if($externalSystemsDown)
            <x-alert tone="error" class="mx-4 text-sm">
                ⚠ {{ __('Shopify or ShipStation is having issues.') }}
                @can('manage-administration')<a href="{{ route('admin.health-incidents') }}" class="underline">{{ __('Details') }}</a>@endcan
            </x-alert>
        @endif
        @if(in_array($group, ['audit', 'search'], true))
            ...
            @endif
        @elseif($contextLinks)<div class="sidebar-section">{{ __(ucfirst($group)) }}</div><ul class="sidebar-nav">@foreach($contextLinks as [$label,$name])@php($names = (array) $name)@if((!in_array($names[0],['jobs.index','print-queue.index'],true) || auth()->user()->can('run-audits')) && (!str_starts_with($names[0],'admin.') || auth()->user()->can('manage-administration')))<li><a class="{{ request()->routeIs(...$names) ? 'active' : '' }}" href="{{ route($names[0]) }}">{{ __($label) }}</a></li>@endif @endforeach</ul>@endif
...
<div class="command-palette" data-command-palette hidden>
    ...
    <section class="command-palette-dialog" role="dialog" aria-modal="true" aria-labelledby="command-palette-title">
        ...
        <div class="command-palette-results" data-command-palette-results role="listbox" aria-label="{{ __('Search results') }}"></div>
        <footer><span>↑↓ {{ __('Navigate') }}</span><span>↵ {{ __('Open') }}</span><span>Ctrl/⌘ K {{ __('Search') }}</span></footer>
    </section>
</div>
@endcan
