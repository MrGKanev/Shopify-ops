...
- [ ] **Readiness endpoint (`/ready`) разширение** — worker freshness (вече
      може да пита Horizon-а, queue-то е решено) и cache readiness проверки,
      след като production cache foundation-ът по-долу се избере.
- [ ] **Structured application logs contract** — production channel/retention
      вече е решено (`daily`, 180 дни); остава единен request/run/store/tool
      ID context, error category/status полета и redaction tests.
