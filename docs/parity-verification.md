...
| Verified mismatch (fixed) | 2 | `dupes` (pair-matching algorithm only); Run Audit comparison engine (classification logic only) |
| Verified mismatch (open) | 1 | Run Audit inline duplicates panel — виж реда по-долу |
| **Not yet audited** | **70 от 72** | Всички други инструменти от `ToolRegistry` |

Последно обновено: 2026-09-14, след pilot-а плюс инцидентната находка за
