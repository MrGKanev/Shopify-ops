 ...
 *
 * Usage:
 *   $cache = new Cache(__DIR__ . '/../cache', ttlSeconds: 14400);
 *   $orders = $cache->remember('ss', "$start|$end", fn() => $api->fetchAll());
 */
class Cache
{
    private string $dir;
    ...
    /**
     * @param string $dir        Absolute path to the cache directory (created if missing).
     * @param int    $ttlSeconds How long a cached entry is valid. 0 = disabled.
     */
     ...
     *
     * @template T
     * @param  string   $prefix  Short label ('ss', 'shopify') — part of filename.
