...
@section('content')
    <div class="flex flex-col gap-6">
        <section><p class="text-sm font-medium text-indigo-600 dark:text-indigo-400">Compliance report</p><h1 class="mt-1 text-3xl font-bold">Email Checker</h1><p class="mt-2 text-slate-500 dark:text-slate-400">Find paid orders with missing, invalid, disposable, or suspicious email addresses.</p></section>
        ...
        @if ($result)
            <section class="flex flex-col gap-4"><h2 class="text-2xl font-bold">{{ $result->scanned }} scanned · {{ $result->critical }} critical · {{ $result->warnings }} warnings</h2>
                ...
                <div class="overflow-x-auto rounded-xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900"><table class="min-w-full divide-y divide-slate-200 text-left text-sm dark:divide-slate-800"><thead><tr><th class="px-4 py-3">Severity</th><th class="px-4 py-3">Order</th><th class="px-4 py-3">Date</th><th class="px-4 py-3">Email</th><th class="px-4 py-3">Issues</th></tr></thead><tbody>
                    @forelse ($result->rows as $row)<tr class="align-top"><td class="px-4 py-3 font-semibold">{{ ucfirst($row['severity']) }}</td><td class="px-4 py-3">@if($row['id'])<a class="text-indigo-600" href="https://{{ $activeStore->shopify_store }}.myshopify.com/admin/orders/{{ $row['id'] }}" target="_blank" rel="noopener noreferrer">{{ $row['number'] }}</a>@else{{ $row['number'] }}@endif</td><td class="px-4 py-3">{{ $row['created_at'] }}</td><td class="px-4 py-3">{{ $row['email'] ?: 'Missing' }}</td><td class="px-4 py-3"><ul class="list-disc pl-5">@foreach($row['issues'] as $issue)<li>{{ $issue['message'] }}</li>@endforeach</ul></td></tr>
                    @empty<tr><td class="px-4 py-8 text-center text-slate-500" colspan="5">No email issues were found in this range.</td></tr>@endforelse
                </tbody></table></div>
            </section>
        @endif
    </div>
